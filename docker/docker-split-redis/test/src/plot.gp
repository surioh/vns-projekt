...
plot "stats/usage.csv" using 1:3 with lines title "Tomcat CPU (%)" lc rgb "red" axis x1y1, \
     ...
     "stats/usage.csv" using 1:6 with lines title "Redis CPU (%)" lc rgb "dark-red" axis x1y1,\
     "stats/usage.csv" using 1:7 with lines title "Redis Memory (%)" lc rgb "dark-blue" axis x1y2
